# Tutorial - NVIDIA NeMo Agent Toolkit with ROS MCP Server

This tutorial explains how to integrate the **ROS MCP Server** using the **NeMo Agent Toolkit** as an MCP client. Through this, users will learn how to configure the MCP client with their preferred model and interact with the ROS MCP Server.
...
The MCP Client configuration file for this example is located at `ros-mcp-server/examples/9_nvidia_nemo/configs/config-mcp-client.yml`. Open this file and modify the following parameters.
...
A modified version of this script is provided at `ros-mcp-server/examples/9_nvidia_nemo/scripts/utils.py`. Replace the original script with the modified version using the following command:

```bash
cp ros-mcp-server/examples/9_nvidia_nemo/scripts/utils.py nemo-agent-toolkit/packages/nvidia_nat_mcp/src/nat/plugins/mcp/utils.py
```
...
A modified version of this script is provided at `ros-mcp-server/examples/9_nvidia_nemo/scripts/client_base.py`. Replace the original script with the modified version using the following command:

```bash
cp ros-mcp-server/examples/9_nvidia_nemo/scripts/client_base.py nemo-agent-toolkit/packages/nvidia_nat_mcp/src/nat/plugins/mcp/client_base.py
``` 
...
cd nemo-agent-toolkit
source .venv/bin/activate
nat serve --config_file /<ABSOLUTE_PATH>/ros-mcp-server/examples/9_nvidia_nemo/configs/config-mcp-client.yml
```
